    ...
    def process_files(self, uploaded_files, meta_template):
        ...
        for file in uploaded_files:
            try:
                ...
                # Scan all sheets for Ra, Rq, Rz, Rt
                        ...
                        for c in range(len(df_sheet.columns)):
                            ...
                            for std_key, keywords in self.targets.items():
                                if any(k in cell_str for k in keywords) and std_key not in row_summary:
                                    # Look Right or Down for the number
                                    val = np.nan
                ...
                # Extract E & F columns from DATA sheet
...
with st.sidebar:
    ...
    st.info("Upload replicates for one sample group at a time.")
...
if not df.empty:
    tabs = st.tabs(["📊 Master Dataset", "📈 Grouped Statistics", "📉 Trend Analysis", "💾 Columnar Export"])

    with tabs[0]:
        st.subheader("Combined Data (Individual Tests)")
    ...
    with tabs[1]:
        ...
        if params:
            ...
            # Scientific Calculation: Mean, SD, CV% per Group
            ...
            # Repeatability Boxplot
            st.write("### Intra-Sample Variance (Replicates)")
            fig_box = px.box(df, x="Sample", y=p_sel, color="Condition", points="all", notched=True)
            st.plotly_chart(fig_box, use_container_width=True)

    with tabs[2]:
        st.subheader("Inter-Sample Trend Analysis")
        if len(df['Day'].unique()) > 1 or len(df['Sample'].unique()) > 1:
            # Calculate 95% Confidence Intervals for Trends
            trend_data = df.groupby(["Sample", "Condition", "Day"])[p_sel].agg(['mean', 'std', 'count']).reset_index()
            trend_data['CI95'] = 1.96 * (trend_data['std'] / np.sqrt(trend_data['count']))
            
            fig_trend = go.Figure()
            for group_label, group_df in trend_data.groupby(["Sample", "Condition"]):
                group_df = group_df.sort_values("Day")
                fig_trend.add_trace(go.Scatter(
                    x=group_df['Day'], y=group_df['mean'],
                    error_y=dict(type='data', array=group_df['CI95'], visible=True),
                    name=f"{group_label[0]} ({group_label[1]})", mode='lines+markers'
                ))
    ...
    with tabs[3]:
        st.subheader("Columnar Export (Side-by-Side)")
        # Flatten all profile data (E & F) into wide format columns
        wide_list = []
        ...
        if wide_list:
            ...
            st.download_button("Download Columnar Profile Data (CSV)", csv, "master_profiles.csv", "text/csv")
else:
    st.info("👋 Start by uploading your first group (e.g., Sample A replicates) in the sidebar.")
