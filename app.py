    ...
    def process_files(self, uploaded_files, metadata_list):
        ...
        for file, meta in zip(uploaded_files, metadata_list):
            try:
                        ...
                        for c in range(len(df_sheet.columns)):
                            ...
                            for std_name, keywords in self.targets.items():
                                if any(k in cell_str for k in keywords) and std_name not in found_params:
                                    ...
                                    if not np.isnan(val): found_params[standard_name] = val
                
                # 2. PROFILE DATA (Col E & F)
                ...
                if found_params: row_summary.update(found_params)
...
if uploaded_files:
    m_id = st.sidebar.text_input("Material ID", "PLA_Sample")
...
if 'summary_df' in st.session_state:
    ...
    # We define 4 tabs here. This must match the code below!
    ...
    with tabs[1]:
        if not prof_df.empty:
            ...
            fig = px.line(subset, x='Length_mm', y='Amplitude_um', title=f"Profile: {f_select}", template="plotly_white")
    ...
    with tabs[3]:
        ...
        with c2:
            ...
            if not prof_df.empty:
                st.write(f"Total Rows: {len(prof_df):,}")
