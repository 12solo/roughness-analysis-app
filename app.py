...
# MAIN TABS (Only show if data exists)
if not st.session_state['summary_df'].empty:
    ...
    with tabs[3]:
        ...
        if prof_dict:
            naming_opt = st.radio("Arrange and Label Columns by:", 
            ...
            for fname, content in prof_dict.items():
                ...
                # Check for duplicate headers (if same material/day uploaded multiple times)
                base_header = header
                counter = 1
                while f"{header}_Length(mm)" in [col for df in wide_dfs for col in df.columns]:
            ...
            st.warning("No 'DATA' sheet profile data found.")
else:
    st.info("👋 Upload your Excel files and click 'Process' to see the analysis.")
